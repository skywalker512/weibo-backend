...
class searchController {
    static async SearchArticle(ctx) {
        ...
        const articles = await ArticleModel.find({content: {$regex: data, $options: '$i'}}).sort({ updatedAt: '-1' }).skip(skip).limit(Number(per_page)).populate('authorId', { name: 1, avatar: 1 }).populate('categoryId', { name: 1 }).lean()
