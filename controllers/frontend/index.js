...
class IndexController {
    static async getIndex(ctx) {
        const article = await ArticleModel.find(null, { createdAt: 0, lastCommentAt:0, changedBy: 0 , praise: 0 }).sort({ updatedAt: '-1' }).limit(10).populate('authorId', { name: 1, avatar: 1 }).populate('categoryId', { name: 1 }).lean();
